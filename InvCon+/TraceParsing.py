...
from plugin.quickNode import fetchAllRuntimeInformation
...
class Simulator:
    ...
    def loadAndexec(self):
        ...
        def auto_fix(tree, rootCall):
            # if len(tree.calls) == len(rootCall.children):
            if structure_equiv(tree, rootCall):
                ...
                if len(rootCall.children) < len(tree.calls):
                    return False
                # if len(rootCall.children) != len(tree.calls):
                #     rootCall.children = list(filter(lambda child: child.exit_frame!=None, rootCall.children))
                #     assert len(rootCall.children) == len(tree.calls)
                k = 0
                matched_children = []
                for i in range(len(tree.calls)): 
        ...
        # Trace2Inv experimental version:                 
        # def auto_fix(tree, rootCall):
        #     if len(tree.calls) == len(rootCall.children):
        #         for i in range(len(tree.calls)):
        #             rootCall.children[i].entry_frame.address = tree.calls[i].address.hex().lower()
        #             auto_fix(tree.calls[i], rootCall.children[i])
        #     else:
        #         rootCall.children = rootCall.children[-len(tree.calls):]
    ...
    def query_contract_calls(self):
        ...
        tx_root_action = self.data["result"]["trace"][0]["action"]
        tx_origin = tx_root_action["from"]
        tx_to = tx_root_action["to"]
        # if tx_to.lower() == self.address.lower():
        #         for child in self.rootCallNode.children:
        #             child.print_state_sstore(self.address)
        #         print("------------------------------------------------")
        #         print(self.rootCallNode.entry_frame.__str__())
        #         for graph  in self.rootCallNode.storage_graphs:
        #             print(hex(graph.root.stack[-1]), hex(graph.root.stack[-2]))
        #             # if hex(graph.root.stack[-1]) == slot:
        #             if True:
        #                 print(json.dumps(graph.__json__(), indent=4))
...
def main():
    ...
    tx_hash = "0x2de6564851f1e48279545d57e1127cafd24a8228488e135994949953905b4ce7"
    
    # 0xa845c3b43fa79a016df0ba85d528f3e32d20bf9134e219493bd2ca62e985bf73
    # 0xbdfe39033b7c171eb24e763ec3e9b5fa16de377f4ce5cd648a208a667b96ec63
    # "0x2de6564851f1e48279545d57e1127cafd24a8228488e135994949953905b4ce7"
    # 0x4b617efd8680ff80b8a39682005fb24a583889dadb3260514aae19017cfbd40e
    # 0xe599460cbba648f47cd24fd4cbdcff4294d64b8fd76929c9c45d49ac03e156ab
