...
class fetcher:
    ...
    def storeTrace(self, category: str, contract: str, Tx: str, FullTrace: bool = False):
        """Given a tx hash, store the trace data"""
        result_dict = self.getTrace(Tx, FullTrace=FullTrace)
        path = getPathFromCategoryTxHash(category, contract, Tx)
        writeCompressedJson(path, result_dict)
...
def main():
    ...
    # fe.storeTrace("FlashSyn", "0x053c80ea73dc6941f518a68e2fc52ac45bde7c9c", HackTx, False )

    # # YearnHackTx = "0xf6022012b73770e7e2177129e648980a82aab555f9ac88b8a9cda3ec44b30779"

    # BuggyTx = "0x5747bc2a2411ce08494b38495db585064666c5d2aba7198e6e1a816b98201d38"
    # fe = fetcher()
    # # trace = fe.getTrace(BuggyTx, FullTrace = True)
    # fe.storeTrace("DeFiHackLabs", "0x250e76987d838a75310c34bf422ea9f1ac4cc906", BuggyTx, True)

# /h/200/zhiychen/TxGuard/Benchmarks_Traces/FlashSyn/Txs/0x7Fcb7DAC61eE35b3D4a51117A7c58D53f0a8a670/
# 0xd770356649f1e60e7342713d483bd8946f967e544db639bd056dfccc8d534d8e.json.gz

    # print(trace)
    # SimpleDAItransferTx = "0x8e0b0a555f2edc487ab775a4e328a09315ba5b8133ffe7f2c4e9d19a89d4c04d"
    # trace = fe.getTrace(SimpleDAItransferTx, FullTrace=True)

    # path = "Harvest1_fUSDT.pickle.gz"
    # writeCompressedJson(path, trace)

    # with open("BuggyTx.json", "w") as f:
    #     json.dump(trace, f, indent=2)

    # traces = fe.batch_getTrace([EminenceHackTx, SimpleDAItransferTx2], FullTrace=True)
    # print( json.dumps(traces, indent = 2) )

    # fe.prettyPrintTrace(trace)
...
## =======================================================================
    ...
    # for _ in range(100):
    #     start = time.time()
    #     writeCompressedJson("temp.json.gz", trace)
    #     end = time.time() - start
    #     end1s.append(end)
    #     readCompressedJson("temp.json.gz")
    #     end = time.time() - (end + start)
    #     end2s.append(end)
    # print("writeCompressedJson time:", sum(end1s)/len(end1s))
    # print("readCompressedJson time:", sum(end2s)/len(end2s))

    # # writeCompressedJson time: 1.068901357650757
    # # readCompressedJson time: 0.45947980642318725
    # # temp.json.gz size: 1.0MB
...
## =======================================================================
    ...
    #     end = time.time() - (end + start)
    #     end2s.append(end)
    # print("pickle dump time:", sum(end1s)/len(end1s))
    # print("pickle load time::", sum(end2s)/len(end2s))
    # # pickle dump time: 0.3972426152229309
    # # pickle load time: 0.3547217416763306
    # # temp.pickle is 30.8 MB
...
#     print("pickle dump time:", sum(end1s)/len(end1s))
#     print("pickle load time::", sum(end2s)/len(end2s))
#     # pickle dump time: 0.7253797173500061
#     # pickle load time: 0.39968705892562867
#     # temp.gz is 1.0 MB
...
#     for _ in range(100):
#         start = time.time()
#         with lzma.open("temp.xz", "wb") as f:
#             pickle.dump(trace, f, protocol=pickle.HIGHEST_PROTOCOL)
#         end = time.time() - start
#         end1s.append(end)
#         temp = None
#         with lzma.open("temp.xz", "rb") as f:
#             temp = pickle.load(f)
#         end = time.time() - (end + start)
#         end2s.append(end)
#     print("pickle dump time:", sum(end1s)/len(end1s))
#     print("pickle load time::", sum(end2s)/len(end2s))
#     # pickle dump time: 2.338243906497955
#     # pickle load time: 0.42037028551101685
#     # temp.xz is  660.1 kB
...
#     for _ in range(100):
#         start = time.time()
#         with lzma.open("temp.dat", "wb") as f:
#             pickled_data = pickle.dumps(trace,  protocol=pickle.HIGHEST_PROTOCOL)
#             compressed_pickle = blosc.compress(pickled_data)
#             f.write(compressed_pickle)

#         end = time.time() - sta    # with open("BuggyTx.json", "w") as f:
    #     json.dump(trace, f, indent=2)rt
#         end1s.append(end)
#         compressed_pickle = None
#         with lzma.open("temp.dat", "rb") as f:
...
#     print("pickle dump time:", sum(end1s)/len(end1s))
#     print("pickle load time::", sum(end2s)/len(end2s))
#     # pickle dump time: 2.286576125621796
#     # pickle load time: 0.6058191728591918
#     # temp.xz is  3.8 MB
