...
def readStats(contractAddress):
    contractAddress = contractAddress.lower()
    path = SCRIPT_DIR + "/cache/" + contractAddress + "/"
    if not os.path.exists(path):
        return None, None
    files = os.listdir(path)
    # delete all json names in files
    files = [f for f in files if f.endswith(".vy")]
    if len(files) != 1:
        sys.exit("vyperAnalyzer: Error: multiple files in the same folder {}!".format(path))
    ...
    with open(path + files[0], "r") as f:
        ...
        # get vyper version
        line = f.readline()
        CompilerVersion = line[8:].strip()
        # get EVM version
        line = f.readline()
        EVMVersion = line[2:].strip()
...
class vyperAnalyzer: 
    ...
    def contract2Sourcecode(self, contractAddress: str):
        ...
        CompilerVersion, EVMVersion = readStats(contractAddress)
