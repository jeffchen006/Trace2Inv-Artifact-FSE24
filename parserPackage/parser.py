...
class VmtraceParser:
    ...
    def parseLogs(self, contractAddress: str, txHash: str, trace):
        ...
        while ii < len(structLogs) - 1:
            ...
            #  calls a method in another contract, using the storage of the current contract
                ...
                # if addr == "0x3c710b981f5ef28da1807ce7ed3f2a28580e0754":
                #     print("now is the time")
            ...
            if structLogs[ii]["op"] == "RETURN":
                ...
                # if self.logging > 0 and self.contractAddressStack[-1].lower() == "0x3f2d1bc6d02522dbcdb216b2e75edddafe04b16f".lower():
                #     print("now is the time")
                calldata = self.calldataStack[-1]["calldata"]

                # if self.contractAddressStack[-1] == "0xb7277a6e95992041568d9391d09d0122023778a2":
                #     print("now is the time")
    ...
    def track(self, contractAddress: str, traceTree, locator: locator,  trace = None): 
        ...
        for ii in range(len(traceTree.internalCalls)):
            # print("ii = {}".format(ii))
            # print(traceTree.internalCalls[ii])
        ...
        while ii < ii_end + 1:
            ...
            # printStack
            # if ii >= 12434:
            #     print("ii = {}".format(ii), structLogs[ii])
            #     Tracker.printStack()
            #     stackStrArr = Tracker.getStack()
            #     if len(stackStrArr) != len(structLogs[ii]['stack']):
            #         sys.exit("Error: len(stackStrArr)({}) != len(structLogs[ii]['stack'])({})".format(
            #             len(stackStrArr), len(structLogs[ii]['stack'])
            #         ))
            # if ii == 13640:
            #     print("now is the time")
...
# 33792 MLOAD 0x140 gets
# 0x23df752323b872dd000000000000000000000000c6028a9fa486f52efd2b95b9
# therefore MLOAD 0x124 gets
# 0x00000000000000000000000000000000000000000000000000002d73|23df7523


# my special logs: 

# 0xa9059cbb
# 0x000000000000000000000000c6028a9fa486f52efd2b95b949ac630d287ce0af00000000000000000000000000000000000000000000000000002e57d75e12eb

            if error == "Error":
                return []
            ii += 1

# break points: [335003, 384109, 384552, 385051]

# 384067 MSTORE overwrite the investedUnderlyingBalance with []

# we need to generate an invariant for one function call inside another function. 
