...
def inferDataFlows(executionTable):
    ...
    for category, benchmarkName, contract, executionList, exploitTx, l1, l2, l3 in executionTable:
        ...
        for ii, (tx, dataS) in enumerate(executionList[counter:]):
            ...
            if benchmarkName in gasBenchmarks:
                ...
                gasDFOverHeadMap[tx] = gasUsed
                gasOBorDFUOverheadMap[tx] = gasUsed
                if key not in gasDFOverHeadMap:
                    gasDFOverHeadMap[key] = gasUsed
            ...
            if benchmarkName in gasBenchmarks and targetFunc in enterFuncs:
                gasOBorDFUOverheadMap[tx] += gasOBOverhead[benchmarkName][0]
            ...
            if tx == exploitTx:
                    ...
                    if dataType == "mapping":
                        ...
                        if "int" in type:
                            if name in invariantMap["dataFlow"] and \
                                ...
                                if benchmarkName in gasBenchmarks:
                                    gasDFOverHeadMap[tx] += gasDFUOverhead[benchmarkName]
                                    
                                    if targetFunc in exitFuncs:
                                        gasOBorDFUOverheadMap[tx] += gasOBorDFUOverhead[benchmarkName]
                                    else:
                                        gasOBorDFUOverheadMap[tx] += gasDFUOverhead[benchmarkName]

                                    gasDFOverHeadMap[key] += gasDFUOverhead[benchmarkName]
                ...
                if len(executionList) == currentIndex + 1 or executionList[currentIndex + 1][0] != exploitTx:
                    ...
                    if benchmarkName in gasBenchmarks:
                        # gasOverheadPath = SCRIPT_DIR + "/cache/gas/{}_DF.json".format(benchmarkName)
                        # writeJson(gasOverheadPath, gasDFOverHeadMap)

                        gasOBorDFUOverheadPath = SCRIPT_DIR + "/cache/gas/{}_OBorDFU.json".format(benchmarkName)
                        # writeJson(gasOBorDFUOverheadPath, gasOBorDFUOverheadMap)
                    break
                ...
                #         if tx not in FPMap["upperBoundsOnTokenTransfers"]:
                #             FPMap["upperBoundsOnTokenTransfers"].append(tx)
                            ...
                            if name in invariantMap["dataFlow"] and \
                                ...
                                if value > maxValue:
                                    if tx not in FPMap["dataFlowUpperBound"]:
                                        FPMap["dataFlowUpperBound"].append(tx)
