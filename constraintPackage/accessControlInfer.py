...
def printFPMap(invariantMap, FPMap, benchmarkName, testingSetSize):
    generatedInvariants = []
...
def inferAccessControl(accesslistTable):    
    ...
    for category, benchmarkName, contract, accessList, exploitTx, l1, l2, l3 in accesslistTable:
        ...
        print("len(training set): ", len(txList))
        ...
        positivePath = SCRIPT_DIR + "/cache/positives/isEOA/{}.txt".format(benchmarkName)
            ...
            if tx == exploitTx:
                ...
                for funcCall in funcCallList[0]:
                    ...
                    if "require(origin==sender)" in invariantMap[name] and invariantMap[name]["require(origin==sender)"]:
                        if benchmarkName in gasBenchmarks:
                            gasEOAOverHeadMap[tx] += gasEOAOverhead[benchmarkName]
                        if origin != sender:
                            isFiltered = True
                ...
                if len(accessList) == currentIndex + 1 or accessList[currentIndex + 1][0] != exploitTx:
                    ...
                    print("FPMap: ", end="")
                    printFPMap(invariantMap, FPMap, benchmarkName, testingSetSize)
