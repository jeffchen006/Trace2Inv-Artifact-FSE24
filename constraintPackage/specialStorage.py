...
def printFPMap(invariantMap, FPMap, benchmarkName):
    ...
    #     newFPMap[invariant] = "{}/{}".format( len(FPMap[invariant]), int(benchmark2txCount(benchmarkName) * (1 - proportion) - 1) )
    # print(newFPMap)

    invariants = ['totalSupply', 'totalBorrow']
    print(invariants)
    for invariant in invariants:
        if invariant in invariantMap:
            print("{}\t".format(len(FPMap[invariant])), end ="" )
        else:
            print("N/A\t", end = "")
    testingSet = int(benchmark2txCount(benchmarkName) * (1 - proportion) - 1)
    trainingSet = benchmark2txCount(benchmarkName) - testingSet
...
def inferSpecialStorage(accesslistTable):    
    ...
    for category, benchmarkName, contract, accessList, exploitTx, l1, l2, l3 in accesslistTable:
        ...
        # if "Harvest2" not in benchmarkName:
        #     continue
        ...
        for tx, funcCallList in accessList:
            ...
            cutoff = benchmark2txCount(benchmarkName) * proportion
            now = len(txList)
        ...
        for slot in specialStorageMap:
            ...
            if tag not in invariantMap:
                ...
                sys.exit("money flow infer2: multiple slots for the same tag")
            ...
            if tx == exploitTx:
                ...
                for funcCall in funcCallList[0]:
                    ...
                    for slot, value, pc in funcCall["sstore"]:
                        ...
                        if "totalSupply" in invariantMap and tag == "totalSupply" and \
                            (value < invariantMap["totalSupply"][0] or value > invariantMap["totalSupply"][1]):
                            isFiltered = True
                            print("Successfully stops the exploit using totalSupply")
                            print("value = {} but the range is {} - {}".format(value, invariantMap["totalSupply"][0], invariantMap["totalSupply"][1]))

                        if "totalBorrow" in invariantMap and tag == "totalBorrow" and \
                            (value < invariantMap["totalBorrow"][0] or value > invariantMap["totalBorrow"][1]):
                            isFiltered = True
                            print("Successfully stops the exploit using totalBorrow")
                            print("value = {} but the range is {} - {}".format(value, invariantMap["totalBorrow"][0], invariantMap["totalBorrow"][1]))

                        # if "totalReserve" in invariantMap and tag == "totalReserve" and \
                        #     (value < invariantMap["totalReserve"][0] or value > invariantMap["totalReserve"][1]):
                        #     isFiltered = True
                        #     print("Successfully stops the exploit using totalReserve")
                        #     print("value = {} but the range is {} - {}".format(value, invariantMap["totalReserve"][0], invariantMap["totalReserve"][1]))
                ...
                for funcCall in funcCallList[0]:
                    ...
                    for slot, value, pc in funcCall["sstore"]:
                        ...
                        if "totalSupply" in invariantMap and tag == "totalSupply" and \
                            (value < invariantMap["totalSupply"][0] or value > invariantMap["totalSupply"][1]):
                            if tx not in FPMap["totalSupply"]:
                                FPMap["totalSupply"].append(tx)
                        
                        if "totalBorrow" in invariantMap and tag == "totalBorrow" and \
                            (value < invariantMap["totalBorrow"][0] or value > invariantMap["totalBorrow"][1]):
                            if tx not in FPMap["totalBorrow"]:
                                FPMap["totalBorrow"].append(tx)

                        # if "totalReserve" in invariantMap and tag == "totalReserve" and \
                        #     (value < invariantMap["totalReserve"][0] or value > invariantMap["totalReserve"][1]):
                        #     if tx not in FPMap["totalReserve"]:
                        #         FPMap["totalReserve"].append(tx)
